...
export default function StudentDashboard() {
  ...
  const [stats, setStats]                   = useState({ exams: 0, avgScore: 0, streak: 0, bookmarks: 0 });
    ...
    const loadData = async () => {
      try {
        ...
        if (sessions.length > 0) {
          const avg = sessions.reduce((s, x) => s + (x.scorePercent || 0), 0) / sessions.length;
          setStats({
            exams:     profile?.totalExams || sessions.length,
            avgScore:  Math.round(avg),
            streak:    profile?.streak || 0,
            bookmarks: profile?.bookmarkCount || 0,
          });
        }
      } catch (e) {
        console.error(e);
  ...
  return (
    <div style={{ padding: '24px', maxWidth: 1200 }}>
      ...
      <div style={styles.statsGrid}>
        {[
          { icon: '📝', label: 'Exams Taken',  value: stats.exams,     color: '#0D9488', bg: 'rgba(13,148,136,0.12)' },
          { icon: '📊', label: 'Avg. Score',    value: `${stats.avgScore}%`, color: '#2563EB', bg: 'rgba(37,99,235,0.12)' },
          { icon: '🔥', label: 'Day Streak',    value: stats.streak,    color: '#F59E0B', bg: 'rgba(245,158,11,0.12)' },
          { icon: '🔖', label: 'Bookmarked',    value: stats.bookmarks, color: '#7C3AED', bg: 'rgba(124,58,237,0.12)' },
        ].map(s => (
